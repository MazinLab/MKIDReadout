...
// compile with gcc -o PacketMaster4 PacketMaster4.c -I. -lm -lrt
...
void Cuber()
{
    ...
    while (access( "/mnt/ramdisk/QUIT", F_OK ) == -1)
    {
       ...
       // flag if we have misaligned data
       //if (oldbr > 808*3) {
       //    errflag = 1;
       //    printf("Processed %d bytes with no start packet!!\n",oldbr); fflush(stdout);
           // print out data to check for a bad packet
       //}
       ...
       while( oldbr >= 808*2 ) {       
          ...
          if( oldbr<808 ) break;  // stop parsing if no more vialbe packets
	...
	/*
          if( errflag == 1 ) {
             printf("Realigning data in Cuber!\n");
             for( i=0; i<oldbr; i++) {
                // look for a byte that is == 255
                if( olddata[i] == 255 ) {
		   newstart = i;
                   printf("Found start at byte %d\n",i);
		   //memcpy();
		   break;  
		}     
              }
