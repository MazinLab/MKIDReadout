...
class IQsweep:
    ...
    def Save(self,filename,roach,wmode):            # Save IQ sweep data into a HDF5 file
        ...
        h5file = open_file(filename, mode = wmode, title = "IQ sweep file created " + time.asctime() )

        # if there is no existing roach group, create one
        try:
            #group = h5file.root.r0
            group = h5file.get_node('/',roach )
        except:
            group = h5file.create_group("/",roach, 'ROACH Identifier' )
        
        # if there is no existing sweep group, create one
        #if group.__contains__('sweeps'):
        #    group = group.sweeps
        #else:
        #    group = h5file.create_group(group,'sweeps', 'IQ sweep data' )

        # make a group for each resonator
        try:
            group = h5file.get_node(group,'f'+str(int(np.float32(self.f0)*10000.0)) )
        except:
            group = h5file.create_group(group,'f'+str(int(np.float32(self.f0)*10000.0)), 'Group Created ' + time.ctime(self.time) )        
        ...
        try:
            ...
            table = h5file.create_table(group,'iqsweep', IQsweeptables, "IQ Sweep Data",filters=filt) 
            swp = table.row
        ...
        if self.savenoise > 0:
            try:
                ...
                try:
                    ...
                    noise = h5file.create_vlarray(group, 'iqnoise', Int16Atom(shape=()), "Noise data stored in ragged array of ints. [0] = I, [1] = Q")
    ...
    def Load(self,filename,roach,f0,atten):            # Load the desired IQ sweep data from a HDF5 file
        ...
        try:
            group = h5file.get_node('/',roach )
            group = h5file.get_node(group,'f'+str(int(f0*10000.0)) )
            table = group.iqsweep
