...
class HighTemplar(QMainWindow):
    def __init__(self, roachNums=None, config='hightemplar.yml'):
        ...
        self.settingsWindow = RoachSettingsWindow(self.roachNums, self.config.roaches, parent=None)  # keep parent None
        #  for now
    ...
    def catchRoachSignal(self, roachNum, command, commandData):
        ...
        if command == RoachStateMachine.DEFINEDACLUT:
            self.sweepWindows[
                roachArg].initFreqs()  # if modified pixel attenuation then need this to remove 'MODIFIED' tag on plot window
            self.sweepWindows[roachArg].updateADCAttenSpinBox(commandData)
