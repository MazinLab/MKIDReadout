...
class RoachSettingsTab(QMainWindow):
    ...
    setDdsShift = QtCore.pyqtSignal(
        int)  # signal emmited when we change the ddsshift so we can reload it into the ROACH2
    initTemplar = QtCore.pyqtSignal(object)

    def __init__(self, roachNum, config):
        super(RoachSettingsTab,
              self).__init__()  # parent=None. This is the correct way according to the QTabWidget documentation
    ...
    def create_main_frame(self):
        ...
        ddsSyncLag = self.config.get('r{}.ddssynclag'.format(self.roachNum))
        ...
        freqFile = self.config.get('r{}.freqfileroot'.format(self.roachNum)) #NB only setting the file base
        ...
        self.textbox_freqFile.textChanged.connect(partial(self.changedSetting, 'freqfileroot'))
        ...
        lofreq = self.config.get('r{}.lo_freq'.format(self.roachNum))
        ...
        self.textbox_lofreq.textChanged.connect(lambda x: self.changedSetting('lo_freq', float(x)))
        ...
        def getInitState():
            ...
            for checkbox in checkbox_commands:
                if checkbox.isChecked():
                    state.append(RoachStateMachine.COMPLETED)
                else:
                    state.append(RoachStateMachine.UNDEFINED)
            return state
        ...
        label_note = QLabel("NOTE: Changing the ip address won't take "
                            "effect until you re-connect. Likewise, changing "
                            "the DDS Sync Lag, freq file, or LO Freq requires "
                            "you to load the freqs/attens again")
...
class FingerTabWidget(QtGui.QTabBar):
    ...
    def paintEvent(self, event):
        ...
        for index in range(self.count()):
            ...
            getLogger(__name__).info("Tab text color: %s", self.tabTextColor(index).name())
