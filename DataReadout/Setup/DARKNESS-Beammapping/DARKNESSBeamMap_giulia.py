...
class StartQt4(QMainWindow):
    ...
    def loadConfigFile(self):
        ...
        self.pixtoCheckFilename = str(self.configData['pixtoCheckFilename'])
        ...
        self.pixtoCheckFile = self.outputDirectory + self.pixtoCheckFilename
        ...
        print 'calculate data for plots...'
        ...
        #Find doubles/pixels to check by eye
        print 'finding doubles and pixels to check...'
        self.writeDoubles()
    ...
    def calculate_plot_data(self):
        ...
        for pixelno in range(self.maximumNumberOfPixels):
            ...
            ###TODO
            self.crx_median[pixelno] = np.mean(np.array(median_array),axis=0) 
            ...
            ###TODO
            self.cry_median[pixelno] = np.mean(np.array(median_array),axis=0)
            #Sep 23 2017: flags modified
            xisBad=self.sweepisBad(self.crx_median[pixelno])
            yisBad=self.sweepisBad(self.cry_median[pixelno])
            if np.logical_and(xisBad,yisBad):
                self.flagarray[pixelno] = 1
            elif xisBad:
                self.flagarray[pixelno] = 3
            elif yisBad:
                self.flagarray[pixelno] = 2
            
            #if np.logical_or(np.sum(self.crx_median[pixelno]) == 0, np.sum(self.cry_median[pixelno]) == 0):
            #    self.flagarray[pixelno] = 1

            ##Sep 2017: modified to flag pixel as bad if max is less or equal to 2 counts (instead of all 0s)
            
            #if np.logical_or(np.amax(self.crx_median[pixelno]) <= 2, np.amax(self.cry_median[pixelno]) <= 2):
            #    self.flagarray[pixelno] = 1
    ...
    def writeDoubles(self):
        totalDoubleCount=0
        xDoubleCount=0
        yDoubleCount=0
        xCheckCount=0
        yCheckCount=0
        print "finding doubles in pixel ",self.pixelStartIndex,"to ", self.pixelStopIndex
        c = open(self.pixtoCheckFile,'w') 
        for pixelno in range(self.maximumNumberOfPixels):
            if self.flagarray[pixelno]!=1: 
                if self.flagarray[pixelno]!=3:
                    xisDouble = self.hasDouble(self.crx_median[pixelno],self.mypeakpos[0][pixelno])
                else: 
                    xisDouble=[False,0]
                if self.flagarray[pixelno]!=2:
                    yisDouble = self.hasDouble(self.cry_median[pixelno],self.mypeakpos[1][pixelno])
                else: 
                    yisDouble=[False,0]
                
                if np.logical_and(pixelno>=self.pixelStartIndex, pixelno<=self.pixelStopIndex):
                
                    if xisDouble[0]:
                        #print "x pix", pixelno,"has double at location:",xisDouble[2], "with height:",xisDouble[3] 
                        self.doublepos[0][pixelno]=xisDouble[2][0]
                        xDoubleCount+=1
                    """
                    if xisDouble[-1]==1:
                        print "x pix", pixelno, "might be noisy"
                        xCheckCount+=1
                    if xisDouble[-1]==2:
                        print  "x pix", pixelno, "has double bigger than max"   
                        xCheckCount+=1
                    """

                    if yisDouble[0]:
                        #print "y pix", pixelno,"has double:",yisDouble[2], "with height:",yisDouble[3]
                        self.doublepos[1][pixelno]=yisDouble[2][0]
                        yDoubleCount+=1
                    
                    """
                    if yisDouble[-1]==1:
                        print "y pix", pixelno, "might be noisy"
                        yCheckCount+=1
                    if yisDouble[-1]==2:
                        print  "y pix", pixelno, "has double bigger than max"   
                        yCheckCount+=1
                    """

                    if xisDouble[-1]!=0:
                        c.write(str(pixelno) + '\t' + "x\t" +  str(xisDouble[-1]) + '\n')
                        
                    if yisDouble[-1]!=0:
                        c.write(str(pixelno) + '\t' + "y\t" +  str(yisDouble[-1]) + '\n')
                
                if np.logical_or(xisDouble[0],yisDouble[0]):
                    totalDoubleCount+=1
        c.close()
        """
        print "total n doubles", totalDoubleCount
        print "total x doubles:", xDoubleCount
        print "total y doubles:", yDoubleCount
        print "x, y to check:", xCheckCount, yCheckCount
        """




    def sweepisBad(self, sweep):
        isBad=False
        if np.logical_or(np.amax(sweep)<=2,np.amax(sweep)-np.mean(sweep)<5.0*np.std(sweep)):
            isBad=True
        return isBad
    
    def hasDouble(self, sweep, mypeakp):
        isDouble=False
        checkFlag=0
        doubleCount=0
        locMax=np.array(argrelextrema(sweep,np.greater))
        threshold=0.5*(np.amax(sweep)-np.mean(sweep))
        doubleInd=locMax[sweep[locMax]-np.mean(sweep)>=threshold]
        peaks=sweep[doubleInd]
        dInd=[]
        dPeaks=[]
        if len(peaks)>1:
            for ind in doubleInd:
                #if len(doubleInd)<5:
                #    print "ind", ind, "height",sweep[ind],"max",np.max(sweep)
                if np.abs(ind-mypeakp)>=5:
                    if doubleCount==0:
                        isDouble=True
                    if (0.9*(sweep[ind]-np.mean(sweep)))>(sweep[int(round(mypeakp))]-np.mean(sweep)):
                        #print "setting flag to 2"
                        checkFlag=2
                    if doubleCount>0:
                        isDouble=False
                        checkFlag=1
            ...
            if isDouble:
                dPeaks=sweep[dInd]
        return [isDouble, doubleCount, dInd, dPeaks, checkFlag]
