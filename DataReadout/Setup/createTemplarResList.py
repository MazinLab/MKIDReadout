...
def createTemplarResList(fn, LO1, LO2, feedline=1, atten=46):
    ...
    """
    ids,_,freqs = np.loadtxt(fn,unpack=True)  # this is in GHz
    print len(freqs)
    ...
    ids1 = ids[np.where(freqs<=LO_avg)]
    freqs2 = freqs[np.where(freqs>LO_avg)]      # on second board
    ids2 = ids[np.where(freqs>LO_avg)]
    
    freqs1 = freqs1[np.where(freqs1 < LO1 + 1.)]
    freqs2 = freqs2[np.where(freqs2 > LO2 - 1.)]
    ids1 = ids1[np.where(freqs1 < LO1 + 1.)]
    ids2 = ids2[np.where(freqs2 > LO2 - 1.)]
    print len(freqs1)
    print len(freqs2)
