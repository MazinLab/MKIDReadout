...
def plotResListData(resList):
    figA = scr.plotResListData(resList,
                                ...
                                fig_size=4,
                                num_cols = 3,
                                plot_fits = [True] * np.ones((6))) #<-- change this to true to overplot the best fits

    plt.show()
 
def plotresSweepParamsVsPwr(resSweep):
    scr.plotResSweepParamsVsPwr(resSweep,
                            plot_keys=['f0','df', 'qi','qc','q0', 'redchi'],
                            num_cols=3)

    plt.show() 

def plotAnaFitEsimates(resSweep, f0s, diff, a_pwr):
...
def plotResListTweaked(resList, plot_types=['IQ'], **kwargs):
        ...
        for temp in temps:
            ...
            if resIndex is not None:
                ...
                for key, ax in axDict.iteritems():
                    ...
                    if key == 'rIQ':
                        ax.plot(res.residualI, res.residualQ, color=plt_color, linestyle='', marker='.',
                    ...
                    if key == 'rMag':
                        ax.plot(scaled_freq, res.resultMag - res.mag, color=plt_color, linestyle='', marker='.',
                                markersize=4, **plot_kwargs)
                    ...
                    if key == 'rPhase':
                        ax.plot(scaled_freq, res.resultPhase - res.phase, color=plt_color, linestyle='', marker='.',
                                **plot_kwargs)
                    ...
                    if key == 'rI':
                        ax.plot(scaled_freq, res.residualI, color=plt_color, linestyle='', marker='.',
                                **plot_kwargs)
                    ...
                    if key == 'rQ':
                        ax.plot(scaled_freq, res.residualQ, color=plt_color, linestyle='', marker='.',
                                **plot_kwargs)
