...
def plotResListData(resList, plot_types=['IQ'], **kwargs):
        ...
        for temp in temps:
            ...
            if resIndex is not None:
                ...
                for key, ax in axDict.iteritems():
                    ...
                    if key == 'rIQ':
                        ax.plot(res.residualI, res.residualQ, color=plt_color, linestyle='', marker='.', markersize=4, **plot_kwargs)
                    ...
                    if key == 'rMag':
                        ax.plot(scaled_freq, res.resultMag-res.mag, color=plt_color, linestyle='', marker='.', markersize=4, **plot_kwargs)
                    ...
                    if key == 'rPhase':
                        ax.plot(scaled_freq, res.resultPhase-res.phase, color=plt_color, linestyle='', marker='.', **plot_kwargs)
                    ...
                    if key == 'rI':
                        ax.plot(scaled_freq, res.residualI, color=plt_color,linestyle='', marker='.', **plot_kwargs)
    ...
    if show_colorbar:
        ...
        # cax = figS.add_subplot(plt_grid[num_rows, :])

        #Plot and label the colorbar
        # cbar_plot = mpl.colorbar.ColorbarBase(cax, orientation ='horizontal', cmap=color_gen, norm=cbar_norm)
