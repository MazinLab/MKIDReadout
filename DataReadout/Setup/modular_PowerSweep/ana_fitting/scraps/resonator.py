...
class Resonator(object):
    ...
    def __init__(self, name, temp, pwr, freq, I, Q, sigmaI = None, sigmaQ = None):
        ...
        # offset = max(res.mag)
        # plt.plot(res.S21-max(res.S21))
        # self.S21 = self.S21-max(self.S21) + 1200
    ...
    def do_lmfit(self, fitFn, **kwargs):
        ...
        # fit_kws= dict(options={'maxiter':10})
        # def count():
        #     print '3'
        #     return True
        ...
        # print 'a', lmfit_result.params['a'].value
        # print type(lmfit_result.params['a'].value)
        ...
        # for i in range(len(self.lmfit_vals)):
        #     print self.lmfit_vals[i], self.lmfit_labels[i]

        # print sum(residualI**2 + residualQ**2)
...
def makeResList(fileFunc, dataPath, resName):
    ...
    fileList = glob.glob(dataPath + resName + '-*' + '*')
