...
def cmplxIQ_fit(paramsVec, freqs, pwr=-50, data=None, eps=None, **kwargs):
    ...
    # kwargs = {'ff': ff, 'f0': f0, 'q0': q0, 'qc': qc, 'pwr': pwr, 'Escale': Escale}
    ...
    # try:
    #     print 'chi2 single', sum(((model-data)/eps)**2)
    # except:
    #     pass
...
def cmplxIQ_params(res, **kwargs):
    ...
    # params.add('f0', value = f0_guess, min = f0_guess-5e-1, max = f0_guess+5e-1, vary=True)
...
def cmplxIQ_fit_cols(paramsVec, freqs, pwr=-50, data=None, eps=None, **kwargs):
    ...
    #
    # try:
    #     print 'chi2 collision', sum(((model-data)/eps)**2)
    # except:
    #     pass
...
def cmplxIQ_params_cols(res, **kwargs):
    ...
    print 'max_a', max_a, 'max_ad', max_ad
    # There shouldn't be any more kwargs left
    if kwargs:
        raise Exception("Unknown keyword argument supplied")
    ...
    params.add('pgain1d', value=phaseBaseCoefs[0], vary=True)
    ...
    params.add('ad', value=0.1, min=0, max=max_ad, vary=True)  # min = 0, max = 10
    return params
