...
def cmplxIQ_fit(paramsVec, freqs, pwr =-50, data=None, eps=None, **kwargs):
    ...
    whole_scope = len(freqs)
    scope_residual = whole_scope-scope
    freq_offset = int(scope_residual/2)
    freqs = freqs[freq_offset:-freq_offset]
...
def cmplxIQ_params(res, **kwargs):
    ...
    # Rupe D added the try/except
    try:
        bandwidth = res.freq[fwhm_mask][-1]-res.freq[fwhm_mask][0]
    except:
        bandwidth = res.freq[-1] - res.freq[0]
    ...
    params.add('f0', value = f0_guess, min = f0_guess-5e-6, max = f0_guess+5e-6, vary=True)
...
def cmplxIQ_fit_cols(paramsVec, freqs, pwr =-50, data=None, eps=None, **kwargs):
    """Return complex S21 resonance model or, if data is specified, a residual.
    ...
    df2 = paramsVec[12] #frequency shift due to mismatched impedances
    f02 = paramsVec[13] #resonant frequency
    qc2 = paramsVec[14] #coupling Q
    qi2 = paramsVec[15] #internal Q
    ...
    #0th and 1st terms in a taylor series to handle phase gain different than 1
    pgain02 = paramsVec[19]
    pgain12 = paramsVec[20]
    ...
    #Allow for voltage offset of I and Q
    offset2 = Ioffset2 + 1j*Qoffset2

    #Calculate model from params at each point in freqs
    # modelCmplx = -gain*pgain*(1./qi+1j*2.0*(ff+df/fs))/(1./q0+1j*2.0*ff)+offset

    modelCmplx = -gain*pgain*(1-q0/(qc*(1+2j*q0*(ff+df))))+offset + -gain2*pgain2*(1-q02/(qc2*(1+2j*q02*(ff+df2))))+offset2
    # plt.plot(modelCmplx)
    # plt.show()
...
def cmplxIQ_params_cols(res, **kwargs):
    """Initialize fitting parameters used by the cmplxIQ_fit function.
    ...
    #This fits a second order polynomial
    magBaseCoefs = np.polyfit(freqEnds, magEnds, 2)
    # magBaseCoefs2 = np.polyfit(freqEnds, magEnds, 2)
    ...
    phaseBaseCoefs = np.polyfit(freqEnds, phaseEnds+phaseRot, 1)
    # phaseBaseCoefs2 = np.polyfit(freqEnds, phaseEnds+phaseRot, 1)
    phaseBase = np.poly1d(phaseBaseCoefs)
    ...
    qi_guess = 0.999*q0_guess*magMax/magMin

    print q0_guess, qi_guess
    qc_guess = 1./(1./q0_guess-1./qi_guess)
    print qc_guess
    ...
    params.add('df2', value = 0, vary=True)
    ...
    params.add('qc2', value = qc_guess, min = 10**5, max = 10**8 ,vary=True)
    params.add('qi2', value = qi_guess, min = 10**5, max = 10**8, vary=True)

    #Allow for quadratic gain variation
    params.add('gain02', value = magBaseCoefs[2], min = 0, vary=True)
    params.add('gain12', value = magBaseCoefs[1], vary=True)
    params.add('gain22', value = magBaseCoefs[0], vary=True)

    #Allow for linear phase variation
    params.add('pgain02', value = phaseBaseCoefs[1], vary=True)
    params.add('pgain12', value = phaseBaseCoefs[0], vary=True)

    #Add in complex offset (should not be necessary on a VNA, but might be needed for a mixer)
    if hardware == 'VNA':
        params.add('Ioffset2', value = 0, vary=False)
        params.add('Qoffset2', value = 0, vary=False)
    elif hardware == 'mixer':
        params.add('Ioffset2', value = 0, vary=True)
        params.add('Qoffset2', value = 0, vary=True)

    params.add('a2', value=0.1,min = 0, max = 5, vary=True)# min = 0, max = 10
    return params
