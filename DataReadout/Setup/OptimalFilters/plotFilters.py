...
class Window(QtGui.QDialog):
    ...
    def initUI(self):  
        ...
        self.check.stateChanged.connect(lambda:self.plot(update=True))
    ...
    def loadData(self):
        ...
        if len(np.shape(self.template_coefficients))==1:
            self.template_coefficients=np.column_stack((self.template_coefficients,np.zeros(len(self.template_coefficients))))
            self.filter_coefficients=np.column_stack((self.filter_coefficients,np.zeros(len(self.filter_coefficients))))
            self.noise_data=np.column_stack((self.noise_data,np.zeros(len(self.noise_data))))
            self.rough_templates=np.column_stack((self.rough_templates,np.zeros(len(self.rough_templates))))
        ...
        self.pixel=0
        self.slider.setMaximum(len(self.file_list)-1)
    ...
    def plot(self,update=False):
        ...
        self.check_type(update=update)
        self.canvas1.draw()
        self.canvas2.draw()
        self.canvas3.draw()

    def check_type(self,update=False):
        if update and self.filter_type[self.pixel]==0:
