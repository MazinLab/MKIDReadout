...
void Cuber(int pfds[2])
{
    ...
    while (access( "/mnt/ramdisk/QUIT", F_OK ) == -1)
    {
	   ...
	   br = read(pfds[0], data, 1024*sizeof(char));
...
void Reader(int pfdsa[2],int pfdsb[2])
{
...
//           inet_ntoa(si_other.sin_addr), ntohs(si_other.sin_port), buf);
    ...
    write(pfdsa[1], buf, nBytesReceived);
    write(pfdsb[1], buf, nBytesReceived);
...
void TestReader(int pfdsa[2],int pfdsb[2])
{
   ...
   while (access( "/mnt/ramdisk/QUIT", F_OK ) == -1) {
      ...
      write(pfdsa[1], data, sizeof(struct datapacket)*(nphot+1));
      write(pfdsb[1], data, sizeof(struct datapacket)*(nphot+1));
      ...
      while (1) {
         clock_gettime(CLOCK_REALTIME, &spec);   
         if( timespec_subtract(&spec,&oldspec) > 0.0000005 ) break;      
      }
