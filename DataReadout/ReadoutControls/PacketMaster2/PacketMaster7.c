...
// global semaphores for locking shared memory.  sem[0] = rptr1, sem[1] = rptr2, sem[2] = rptr3
static sem_t sem[3];

//#define LOGPATH "/mnt/data0/logs/"

// compile with gcc -Wall -Wextra -o PacketMaster7 PacketMaster7.c -I. -lm -lrt -lpthread -O3

uint8_t boardNums[NROACH] = {0};
...
/*
 * Same as ParsePacket, but image is transposed to fit opencv convention
 */
void ParsePacketSN( uint16_t *image, char *packet, unsigned int l, uint64_t frame[NROACH])
{
    uint64_t i;
    //struct hdrpacket *hdr;
    struct datapacket *data;
...
*/
    ...
    for(i=1;i<l/8;i++) {
       ...
       if( data->xcoord >= XPIX || data->ycoord >= YPIX ) continue;
       (*(image + (data->ycoord)*XPIX + data->xcoord))++; //Rows are stored sequentially in image array
...
void Nuller()
{
    ...
    uint32_t doneIntMask = (1<<NROACH)-1; //each place value corresponds to a roach board
    int takingImg = 0;
    int fd;
    int semVal;
    ...
    if (ftruncate(fd, sizeof(uint16_t)*XPIX*YPIX) == -1) { 
        perror("ftruncate");  /* something went wrong */
        exit(1);               
    }
  
    imgBuffPtr = mmap(NULL, sizeof(uint16_t)*XPIX*YPIX, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
void Reader()
{
  ...
  while (access( "/mnt/ramdisk/QUIT", F_OK ) == -1)
  {
    ...
    */
    ...
    // write the socket data to the three shared memory spaces
    sem_wait(&sem[0]);       
    ...
    if( rptr3->unread >= (SHAREDBUF - BUFLEN) ) {
       perror("Data overflow 3 in Reader.\n");   
    } 
    else {
...
int main(void)
{
    ...
    rc = pthread_create(&threads[3], &attr, Nuller, (void *) &t);
    if (rc){
