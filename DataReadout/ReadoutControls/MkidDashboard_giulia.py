...
class MkidDashboard(QMainWindow):
    ...
    def addFlatImage(self, photonImage):
        tempSavePath='/home/kids/giulia/'
        if self.checkbox_darkImage.isChecked() and self.darkField is not None:
            np.savez(tempSavePath+'photonImage_noDarkSub_'+str(self.tempFlatVar)+'_img_'+str(self.takingFlat)+'.npz',photonImage)
            photonImage = np.array(photonImage, dtype=np.float) - self.darkField
        if self.takingFlat==1:
            np.savez(tempSavePath+'darkImage_flat_'+str(self.tempFlatVar)+'.npz',self.darkField)
        self.spinbox_flatImage.setEnabled(False)
        
        if self.flatField is not None: 
           np.savez(tempSavePath+'flatField_'+str(self.tempFlatVar)+'_img_'+str(self.takingFlat)+'.npz',self.flatField)
           self.flatField+=photonImage
        
        else:
            print "flat field=None for self.takingFlat=",self.takingFlat
            np.savez(tempSavePath+'flatField_initialFlat_'+str(self.tempFlatVar)+'_img_'+str(self.takingFlat)+'.npz',self.flatField)
            self.flatField=photonImage
        np.savez(tempSavePath+'photonImage_flat_'+str(self.tempFlatVar)+'_img_'+str(self.takingFlat)+'.npz',photonImage)
        print "counting", self.takingFlat
        self.takingFlat-=1
        if self.takingFlat ==0:
            ...
            np.savez(tempSavePath+'beamMapFailed_'+str(self.tempFlatVar)+'.npz',self.beammapFailed) 
            ...
            np.savez(tempSavePath+'weights'+str(self.tempFlatVar)+'.npz',self.flatField)
            print "flat median, average", flatMedian, np.mean(self.flatField[np.where(self.beammapFailed==0)])
